...
const PaymentSuccess: React.FC = () => {
    ...
    const failPayment = async (event: React.FormEvent<HTMLFormElement>) => {
        event.preventDefault()
        let id = localStorage.getItem("reservationId") ?? 0;
        if (id === 0) return;

        fetch(process.env.SERVER_DOMAIN + "/reservation/failReservationPayment?Id=" + id, {
            method: 'POST',
            credentials: 'include'
        })
            .then(response => {
